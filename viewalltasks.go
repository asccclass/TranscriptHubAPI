...
func (app *TranscripthubService) getAllTasks(sso string) ([]Task, error) {
	query := `SELECT objid, filename, label, sso_account, status, diarize, created_at, updated_at FROM tasks WHERE sso_account = ?`
	...
	for rows.Next() {
		...
		if err := rows.Scan(&task.OBJID, &task.Filename, &task.Label, &task.SSOAccount,
			&task.Status, &task.Diarize, &task.CreatedAt, &task.UpdatedAt); err != nil {
			return nil, err
		}
